...
// ===== PRODUCT DATA FETCHED FROM FIREBASE =====
let products = [];
...
async function fetchProductsFromFirebase() {
    ...
    try {
        if (typeof firebase === 'undefined') {
            ...
            return; // Fallback to empty or show error
        }
        ...
        products = snapshot.docs.map(doc => {
            ...
            // Map Firestore data to local structure if needed
            // The Admin Panel saves data in a structure that includes 'variants'
            // We need to ensure it matches what the UI expects:
            // - colors: array of strings
            // - sizes: array of strings
            // - colorImages: object { color: url }
            // - sizePrices: object { size: price } (optional override)

            // Re-construct colorImages and sizePrices from variants if available
                ...
                data.variants.forEach(variant => {
                    ...
                    if (variant.sizes && Array.isArray(variant.sizes)) {
                        variant.sizes.forEach(s => {
                            if (s.price) {
                                // Start assuming price is unique per size, 
                                // but if size exists in multiple colors with diff prices, this frontend logic might be too simple.
                                // For now, take the first one or average.
...
document.addEventListener('DOMContentLoaded', () => {
    // Initialize Firebase products fetch
    if (typeof fetchProductsFromFirebase === 'function') {
        fetchProductsFromFirebase();
    } else {
        loadProducts(); // Fallback if function missing
    }
...
function encodeImagePath(path) {
    // Encode spaces and special characters in folder/file names
...
function createProductCard(product) {
    ...
    return `
        <div class="product-card" data-category="${product.category}" data-product-id="${product.id}" onclick="viewProductDetail(${product.id})" style="cursor: pointer;">
            ...
            <div class="product-info">
                ...
                <div class="product-variants">
                    <div class="size-selector">
                        <label>Size:</label>
                        <select class="product-size" id="size-${product.id}" onclick="event.stopPropagation();" onchange="updatePriceForSize(${product.id}, this.value)">
